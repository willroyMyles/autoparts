...
class CarData {
  List<String> makes = [];
  List<List<dynamic>> models = [];
  List<String> category = [];

  CarData() {
    var list = processCars();
    makes = list.map((e) => e.first).toList().cast<String>() as List<String>;
    models = list;
    category = processList().cast<String>();
  }

  List<dynamic> getModels(int index) {
    var list = models.elementAt(index);
    return list;
  }
}

CarData _carData = CarData();
CarData get serviceCarData => _carData;

List<dynamic> processList() {
  var parts =
      '''Engine/Transmission/Wheels & Rims/Body Parts/Accessories/Msic''';
...
Zhujiang''';
  ...
  var list = CsvToListConverter(fieldDelimiter: ",", eol: "\n").convert(cars);
